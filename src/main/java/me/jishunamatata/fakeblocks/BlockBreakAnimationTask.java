...
public class BlockBreakAnimationTask extends BukkitRunnable {
	...
	public void run() {
		for (Entry<EntityPlayer, BlockAnimationContext> entry : blockMap.entrySet()) {
			...
			float f = 0;
			try {
				f = (float) BREAK_STATUS.invoke(player.playerInteractManager, data, context.getBlockPos(),
						LAST_DIG_TICK.get(player.playerInteractManager));
			} catch (ReflectiveOperationException e) {
				e.printStackTrace();
			}
